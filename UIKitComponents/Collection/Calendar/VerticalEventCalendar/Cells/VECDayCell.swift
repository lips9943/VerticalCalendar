...
//        resetAllEventViews()
    ...
    private func dayColor(model: VECDay) {
        if model.isSaturday {
            ...
            dayLabel.backgroundColor = VECDayCell.todayCellColor
            dayLabel.textColor = VECDayCell.todayLabelColor
        }else {
...
extension VECDayCell {
    ...
    private func setupEventView(at index: Int, with event: VECEvent, when day: Date, showLabel: Bool, multiple: CGFloat) {
        ...
        if day.year == 2025, day.month == 5, day.day == 21 {
...
extension VECDayCell {
    ...
    private func applyLayout(for eventView: UIView, at index: Int, multiple: CGFloat) {
        let topOffset: CGFloat = contentView.bounds.height * 0.06 // 첫 번째 이벤트와 날짜 레이블 사이 간격
