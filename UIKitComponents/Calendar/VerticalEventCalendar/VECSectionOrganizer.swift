...
struct VECSectionOrganizer {
    ...
    func applyEventsOnEachDayFromStartToDeleteEvents(events: [VECEvent], section: VECSection) -> VECSection {
        var section = section
        for (index, day) in section.days.enumerated() {
            var newDay = day
            var resetedEvents: [VECEvent?] = [nil, nil, nil, nil]
            newDay.events = resetedEvents
            for event in events {
